...
SDL_Renderer *GameEngine::getRenderer() { return m_renderer; }

StateManager *GameEngine::getStateManager() { return m_stateManager; }
...
GameEngine::GameEngine(int width, int height, int f)
	: screen_width(width), screen_height(height), fps(f) {
	...
	if (SDL_Init(SDL_INIT_VIDEO) < 0) {
		...
		m_window = SDL_CreateWindow(
			"The worst Space Invaders game ever...", SDL_WINDOWPOS_UNDEFINED,
			SDL_WINDOWPOS_UNDEFINED, screen_width, canvas_height, SDL_WINDOW_SHOWN);
	...
	if ((TTF_Init() <
		0)) { // std::cout << "could not initialize sdl_ttf" << std::endl;
		exit(1);
	}
...
int GameEngine::getScreenWidth() { return screen_width; }

int GameEngine::getScreenHeight() { return screen_height; }

int GameEngine::getCanvasHeight() { return canvas_height; }

int GameEngine::getScore() { return m_score; }

void GameEngine::addToScore(int addend) { m_score += addend; }

TTF_Font *GameEngine::getFont() const { return font; }